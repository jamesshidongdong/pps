...
namespace AlwaysPPS.Web.Controllers
{
    ...
    public class ReportFormController : BaseController
    {
        ...
        private readonly IReportCharDataService _charData;

        private readonly IReportDataByClient _client;

        private readonly IReportDataByProject _projectData;

        private readonly IReportDataByDept _deptData;
        //constructor
        ...
        public ActionResult GetTeams()
        {
            ...
            foreach (var teamID in teamIDs)
            {
                ...
                var ctModel = _iCtTeamService.GetListByTeamID(teamID);
                if (ctModel!=null)
                {
                    res.Add(ctModel);
        ...
        public ActionResult GetReportAll()
        {
            return View();
        }

        [HttpGet]
        public ActionResult GetCustomersBydept()
        {
          var res=  _projectService.GetClienNameByType();
          if(res.Any())
          {
              var jsonRes=res.Select(u => new SelectListItem() {Text = u, Value = u}).ToList();
              return Json(jsonRes, JsonRequestBehavior.AllowGet);
          }
            return null;
        }
        ...
        public  ActionResult GetProjects()
        {
           ...
           var res= _projectService.GetProjectsByType();
            if(res.Any())
            {
                var jsonRes =
                    res.Select(u => new SelectListItem() {Text = u.ProjectName, Value = u.ProjectName.ToString()});
                return Json(jsonRes, JsonRequestBehavior.AllowGet);
            }
            return null;
        }
        ...
        public ActionResult GetReportData(ReportSearchModel model)
        {

            int pageIndex = model.page.HasValue ? model.page.Value : 1;
            int pageSize = model.pageSize.HasValue ? model.pageSize.Value : 15;
            DateTime sTime = DataHelper.CalculateFirstDateOfWeek(DateTime.Now);
            DateTime eTime = DataHelper.CalculateLastDateOfWeek(DateTime.Now);
            DateTime startDate = model.StartData.HasValue ? model.StartData.Value : sTime;
            DateTime endDate = model.EndData.HasValue ? model.EndData.Value : DateTime.Now;
            model.StartData = startDate;
            model.EndData = endDate;
            model.page = pageIndex;
            model.pageSize = pageSize;
           var res= _charData.GetData(model);
           DataSourceResult data = new DataSourceResult();
            data.Data = res;
            data.Total = res.Any() ? res[0].TotalCount : 0;
            return Json(data);
        }

        public ActionResult GetReportDataByClient(ReportSearchModel model)
        {
              DateTime sTime = DataHelper.CalculateFirstDateOfWeek(DateTime.Now);
            DateTime eTime = DataHelper.CalculateLastDateOfWeek(DateTime.Now);
            DateTime startDate = model.StartData.HasValue ? model.StartData.Value : sTime;
            ...
            model.StartData = startDate;
            model.EndData = endDate;
          var clientRes=  _client.GetData(model);


           return Json(clientRes, JsonRequestBehavior.AllowGet);
        }
        public ActionResult GetReportDataByDept(ReportSearchModel model)
        {
            DateTime sTime = DataHelper.CalculateFirstDateOfWeek(DateTime.Now);
            DateTime eTime = DataHelper.CalculateLastDateOfWeek(DateTime.Now);
            DateTime startDate = model.StartData.HasValue ? model.StartData.Value : sTime;
            ...
            model.StartData = startDate;
            model.EndData = endDate;
            var deptRes = _deptData.GetData(model);


            return Json(deptRes, JsonRequestBehavior.AllowGet);
        }
        public ActionResult GetReportDataByProject(ReportSearchModel model)
        {
            DateTime sTime = DataHelper.CalculateFirstDateOfWeek(DateTime.Now);
            DateTime eTime = DataHelper.CalculateLastDateOfWeek(DateTime.Now);
            DateTime startDate = model.StartData.HasValue ? model.StartData.Value : sTime;
            ...
            model.StartData = startDate;
            model.EndData = endDate;
            var projectRes = _projectData.GetData(model);


            return Json(projectRes, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult ExportSummaryReport(ReportSearchModel model)
        {
          //model.StartData=  Convert.ToDateTime(Request.Params["StartData"]);
          //model.EndData = Convert.ToDateTime(Request.Params["EndData"]);
          //model.createDeptid = Request.Params["createDeptid"];
          //model.clientName = Request.Params["clientName"];
          //model.projectName = Request.Params["projectName"];
            DateTime sTime = DataHelper.CalculateFirstDateOfWeek(DateTime.Now);
            DateTime eTime = DataHelper.CalculateLastDateOfWeek(DateTime.Now);
            DateTime startDate = model.StartData.HasValue ? model.StartData.Value : sTime;
            DateTime endDate = model.EndData.HasValue ? model.EndData.Value : DateTime.Now;
            ...
            var res = _charData.GetData(model);
            ExcelPackage excel=new ExcelPackage();
            ExcelWorksheet sheet = excel.Workbook.Worksheets.Add("工时汇总");
            sheet.Cells[1, 1].Value = "客户";
            sheet.Cells[1, 2].Value = "立项部门";
            sheet.Cells[1, 3].Value = "项目名称";
            sheet.Cells[1, 4].Value = "总工时";
            for (int i = 0; i < res.Count; i++)
            {
                sheet.Cells[i + 2, 1].Value = res[i].ClientName;
                sheet.Cells[i + 2, 2].Value = res[i].DepartmentName;
                sheet.Cells[i + 2, 3].Value = res[i].ProjectName;
                sheet.Cells[i + 2, 4].Value = res[i].Totalhours.ToString();
            }
            var resArr = excel.GetAsByteArray();
            return File(resArr, "application/vnd.ms-excel", HttpUtility.UrlEncode("Excel.xlsx"));
        }
    }
}